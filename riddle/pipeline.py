...
def run(args, contract):
    ...
    for partition, suffix in (("validation", "val"), ("test", "test"), ("signal_region", None)):
        ...
        rows = np.vstack(ordered_map(np.load, [args.data / name for name in names], args.io_workers)).astype(
            "float32"
        )
    ...
    write_json(
        output / "protocol.json",
        {
            ...
            **settings["riddle"]["training"],
            ...
            "ensemble": f"equal-weight mean signal density over valid fits and {settings['riddle']['training']['selected_checkpoints']} validation-selected epochs per fit",
            "settings": settings,
